...
class VectorSearchService:
    ...
    async def reindex_all(self, group_id: Optional[str] = None) -> int:
        ...
        M6: 持有 reindex_lock 防止与 index_quote/remove_quote 竞态。
        """
    ...
    async def is_available(self) -> bool:
        """M4: 检查向量搜索基础设施是否就绪（store 已连接、embedding 可用）。

        不再检查索引是否有数据，调用方应通过 get_index_count() 区分"索引为空"。
        """
        try:
