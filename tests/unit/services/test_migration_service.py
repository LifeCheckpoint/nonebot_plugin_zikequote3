...
class TestExecuteMigration:
    ...
    async def test_migration_move_mode(
        self,
        migration_service: MigrationService,
        mock_quote_repo: AsyncMock,
        mock_group_member_repo: AsyncMock,
        ...
        await migration_service.execute_migration(
            [_make_quote("1")], "src", "tgt", keep_source=False
        )
        # Should delete source quotes in move mode
        calls = [c.args for c in mock_quote_repo.delete_quotes_by_group.await_args_list]
        assert ("src",) in calls
